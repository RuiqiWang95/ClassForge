...
if __name__ == '__main__':
    ...
    dloader_val = FewShotDataloader(
        ...
        nKnovel=5,
        nKbase=2,
        nExemplars=2,  # num training examples per novel category
        nTestNovel=5 * 6,  # num test examples for all the novel categories
        nTestBase=2*6,  # num test examples for all the base categories
        batch_size=1,
        num_workers=0,
        epoch_size=1,  # num of batches per epoch
    )

    for d in dloader_val(0):
        for i in d:
            print(i.size())
            if len(i.size()) <3 :
                print(i)
